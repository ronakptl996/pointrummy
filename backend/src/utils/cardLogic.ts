...
const sortCard = (cardsArr: string[]) => {
  ...
  numArr.map((ele, ind): void => {
    numArr_with_index.push([ele, ind]);
  });

  numArr_with_index.sort((a, b) => {
    return a[0] - b[0];
  });

  numArr_with_index.map((ele, ind): void => {
    index.push(Number(numArr_with_index[ind][NUMERICAL.ONE]));
  });

  index.map((ele, ind): void => {
...
const isPure = (card: string[]): any => {
  ...
  card.map((ele, ind) => {
    let arr = ele.split("_");
    typeCard.push(arr[NUMERICAL.ONE]);
    numArr.push(Number(arr[NUMERICAL.ONE]));
  ...
  if (typeCard.includes("J")) return false;
    ...
    if (
      card.length > NUMERICAL.TWO &&
      res.indexOf(false) == res.length - NUMERICAL.ONE &&
      cardLatter
    )
      return true;
      ...
      if (index) {
        ...
        for (let i = NUMERICAL.ZERO; i < card.length; i++) {
          newCard.push(
            `${typeCard[i]}_${numArr[i]}_${typeArr[i]}_${deckNo[i]}`
          );
        }
...
const isImpure = (card: string[]): any => {
  let typeCard: string[] = [];
  let numArr: string[] = [];
  let typeArr: any[] = [];
  let deckNo: any[] = [];
  ...
  const cardLatter = typeCardWithOutJW.every(
    (val) => val === typeCardWithOutJW[NUMERICAL.ZERO]
  );
  let numberLatter = numArrWithOutJW.some(
    (val, i) => numArrWithOutJW.indexOf(val) !== i
  );
  numArrWithOutJW.sort(function (left, right) {
  ...
  if (numberLatter) {
    ...
    (card.length > NUMERICAL.TWO &&
      !numberLatter &&
      cardLatter &&
      cardJW.length >= gapCount) ||
    (card.length > NUMERICAL.TWO && cardLatter && gapCount == NUMERICAL.ZERO)
  ) {
    return true;
  } else {
...
const isSet = (card: string[]) => {
  ...
  const allNumberSame = numArrWithOutJW.every(
    (val) => val === numArrWithOutJW[0]
  );
  ...
  if (
    card.length > NUMERICAL.TWO &&
    allNumberSame &&
    allSameSuitCount_S < NUMERICAL.TWO &&
    allSameSuitCount_C < NUMERICAL.TWO &&
    allSameSuitCount_H < NUMERICAL.TWO &&
    allSameSuitCount_D < NUMERICAL.TWO &&
    card.length < NUMERICAL.FIVE
  ) {
    return true;
  } else {
    return false;
