...
import cancelScoreBoardTimer from "../../../scheduler/cancelJob/cancelScoreBoardTimer.cancel";
...
import cancelShowScoreBoardTimer from "../../../scheduler/cancelJob/showScoreBoardTimer.cancel";
import { removeQueue } from "../../common/queue";
import emitLeaveTableEvent from "./emitLeaveTableEvent";

const manageLeaveTableInWinnerDeclare = async (
  userId: string,
  tableId: string,
  socketId: string,
  ...
  try {
    Logger.info(
      " manageLeaveTableInWinnerDeclare :: Starting  :: userId  ::",
      userId,
      "tableId :: ",
      tableId,
      "socketId :: ",
    ...
    Logger.info(
      tableId,
      ` leaveTable :: tableState :: ${tableGamePlay.tableState}`
    );

    // seat user state change
    const updateSeats: ISeats[] = [];
    let userCount = 0;
    ...
    for await (const player of updateSeats) {
      if (
        player.userState !== PLAYER_STATE.QUIT &&
        player.userState !== PLAYER_STATE.DISCONNECTED
      ) {
        Logger.info(
          tableId,
    ...
    Logger.info(
      tableId,
      "- leaveTable :: currentPlayerInTable",
    ...
    if (tableGamePlay.tableState === TABLE_STATE.WINNER_DECLARED) {
      ...
      if (userCount === NUMERICAL.ZERO) {
        Logger.info(
          tableId,
          ` leaveTable :: currentPlayerInTable :: ${tableGamePlay.currentPlayerInTable}`
        );

        await cancelShowScoreBoardTimer(
          `scoreBoard:${tableId}:${NUMERICAL.ONE}`,
          tableId
        );
        ...
        for await (const player of updateSeats) {
          await playerGamePlayCache.deletePlayerGamePlay(
            player.userId,
            tableId
          );
        }
    ...
    Logger.info(
      " manageLeaveTableInWinnerDeclare :: Ending  :: userId  ::",
      userId,
      "tableId :: ",
      tableId,
      "socketId :: ",
    ...
    Logger.error(
      tableId,
      "<<======= manageLeaveTableInWinnerDeclare() Error ======>>",
      error
    );
...
export default manageLeaveTableInWinnerDeclare;
