...
import { EVENT, TABLE_STATE, NUMERICAL } from "../../constants";
...
const { GAME_START_TIMER, LOCK_IN_TIMER } = config.getConfig();

const roundStartTimer = async (
  tableId: string,
  currentRound: number,
  ...
  try {
    Logger.info(
      tableId,
      `Starting roundStartTimer for tableId : ${tableId} and round : ${currentRound}`
    );
    ...
    if (tableGamePlay.currentPlayerInTable >= tableConfig.minPlayer) {
      ...
      if (tableConfig.noOfPlayer === NUMERICAL.TWO) {
        countDown = Number(GAME_START_TIMER);
      }

      Logger.info("roundStartTimer ::: countDown :::: ", countDown);
      const countDownData: ICountDown = {
        time: countDown / NUMERICAL.THOUSAND,
        tableId,
      };

      const validatedCountDownData: ICountDown =
        await countDownResponseFormator(countDownData);

      Logger.info(tableId, " CountDownData :: ", validatedCountDownData);

      commonEventEmitter.emit(EVENT.GAME_COUNT_DOWN, {
        tableId,
    ...
    Logger.info(
      tableId,
      `Ending roundStartTimer for tableId : ${tableId} and round : ${currentRound}`
    );
    return false;
  } catch (error) {
    Logger.error(
      tableId,
      error,
      ` table ${tableId} round ${currentRound} function roundStartTimer`
    );
    throw new Error(`roundTimerStart() ${error}`);
  }
};

export default roundStartTimer;
