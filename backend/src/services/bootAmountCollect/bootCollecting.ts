...
const { BOOT_COLLECT_TIMER } = config.getConfig();

const bootCollecting = async (
  tableId: string,
  currentRound: number,
  ...
  try {
    Logger.info(
      tableId,
      `Starting bootCollecting for tableId : ${tableId} collectAmountFlag : ${collectAmountFlag}`
    );
    ...
    if (collectAmountFlag) {
      firstRoundBootCollectFlag = await firstRoundBootCollect(
        tableId,
        currentRound
      );
    }
    ...
    Logger.info(
      tableId,
      `bootCollecting ::: firstRoundBootCollectFlag :: `,
      firstRoundBootCollectFlag
    );
